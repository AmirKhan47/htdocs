...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <a href="<?php echo RURL.'leads/detail/'.$leads['id'];?>">leads Detail</a>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <input type="file" class="form-control chk2" id="measurement" name="measurement"  data-validation="required"  multiple="multiple">
                                                        ...
                                                        <button type="submit" class="btn btn-info chk2" id="upload"><i class="fa fa-upload"></i> Upload</button>
                                                        <button type="button" class="btn btn-success chk3" id="visit_submit" name="visit_submit"><i class="fa fa-arrow-circle-right"></i> Submit</button>
                                                        <button type="button" class="btn btn-danger chk3" id="visit-fail"><i class="fa fa-exclamation-triangle"></i> fail</button>
                                                        <!-- End of visit form -->
...
<script>
    ...
    if (!checkedNum){
        ...
        $.ajax({
            ...
            success:function(data){
                if(data==1){
                    $("#visit-form")[0].reset();
                    // $('input[name="measurement"]')[0].files.val('');
                    visit_table(1);
                }else if(data==3){
  ...
  $('#pre_upload').click(function(e){
    e.preventDefault();
    var data = new FormData();

    var form_data = $('#pre-form').serializeArray();
    $.each(form_data, function (key, input) {
        data.append(input.name, input.value);
    });

     //input quatation
    var quotation_wip = $('input[name="quotation_pre"]')[0].files;
    if(quotation_wip.length>0){
        for (var i = 0; i < quotation_wip.length; i++) {
           data.append("quotation_pre[]", quotation_wip[i]);
        }
  ...
  $('#continue-submit').click(function(e){
    ...
    $.ajax({
        url:url+'lead/submit_lead_progress',
        type:'POST',
        data:serialized,
        success:function(data){
            // alert(data);exit;
  ...
  function changestatus(){
    ...
    }else if(status2 == 'Fail' || status2 == 'Success'){
        $('#visit-list').addClass('active');
        $('#portlet_tab1').addClass('active');
        $('#pre-list').removeClass('active');
