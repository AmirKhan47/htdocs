...
//end get_all_countries  
	...
	{
   		$result = $this->db->insert_batch('excel',$data);
        if($result)
        {
            return TRUE;
        }
    ...
    public function all_registered_students_data()
	{
		$this->db->select('*');
	    ...
	    $this->db->join('fee_structure fs','fs.class_id=r.class_id','left');
	...
	public function field_check($table='',$column='',$field='')
	{
		$this->db->select(array($column)); 
     	$this->db->from($table);
