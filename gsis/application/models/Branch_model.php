...
class Branch_model extends CI_Model
{
	...
	public function get_branches($id=0)
	{
		$column_order = array('b.branch_code','b.branch_name','b.created_at','a.address_local','c.contact_email','c.contact_cell'); //set column field database for datatable orderable
		$column_search = array('b.branch_code','b.branch_name','b.created_at','a.address_local','c.contact_email','c.contact_cell'); //set column field database for datatable searchable 
		$order = array('b.id' => 'desc'); // default order 
		$this->db->select('b.*,a.address_local,c.contact_email,c.contact_cell');
		$this->db->from('branch as b');
		$this->db->join('branch_contact as bc', 'bc.branch_id = b.id', 'left');
		$this->db->join('address as a', 'a.contact_id = bc.contact_id', 'left');
		$this->db->join('contact as c', 'c.id = bc.contact_id', 'left');
        $i = 0;
        if($id==0)
        {
