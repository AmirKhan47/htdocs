...
class Level_model extends CI_Model
{
	...
	{
		$column_order = array('level_name','created_at'); //set column field database for datatable orderable
		$column_search = array('level_name','created_at'); //set column field database for datatable searchable 
		$order = array('level_id' => 'desc'); // default order 
		$this->db->select('*');
		$this->db->from('levels');
        $i = 0;
        if($id==0)
