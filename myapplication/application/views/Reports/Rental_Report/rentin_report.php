...
$(document).ready(function() {
  ...
  function urlReload(){
      table.ajax.url("<?php echo site_url('Reports/populateRentIn')?>/"+from_date+"/"+to_date+"/"+cust.trim()+"/"+type.trim()).load();
  }
  ...
  function calculateTotals(){
    var qty = table.column( 3 ).data().sum();
    var amount = table.column( 4 ).data().sum();
    alert(qty + " " + amount);
  }
  table = $('#table').DataTable({ 
      // Load data for the table's content from an Ajax source
      "ajax": {
          "url": "<?php echo site_url('Reports/populateRentIn')?>/"+from_date+"/"+to_date+"/"+cust+"/"+type,
          "type": "POST"
      },
      buttons: [
